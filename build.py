...
function fmtWith(v,d){ return v.toLocaleString(locale(),{minimumFractionDigits:d,maximumFractionDigits:d}); }
...
function draw(){
  ...
  let series=meta.lines.map(L=>({label:L.label,color:L.color,unit:meta.unit,decimals:meta.decimals,pts:applyTransform(aggregate(L.points,state.gran,meta.agg),state.transform)}));
  ...
  if(cmpId){
    ...
    if(state.compareMode==="spread"){
      // Ecart calcule en unites mixtes si meta.unit != cm.unit ; on l'affiche neutre pour ne pas induire en erreur.
      const spreadUnit = meta.unit===cm.unit ? meta.unit : "";
      series=[{label:t("spreadLabel")+" (" + meta.name + t("minusStr") + cm.name + ")", color:CMP_COLOR, unit:spreadUnit, decimals:meta.decimals, pts:spreadPts(prMain.pts, cmPts)}];
    } else {
      series=series.concat([{label:cm.name, color:CMP_COLOR, unit:cm.unit, decimals:cm.decimals, pts:cmPts, dashed:true}]);
    }
  }
  renderCorr(corr);
...
function onMove(e){
  ...
  for(const s of series){
    ...
    const sd = (s.decimals!=null) ? s.decimals : meta.decimals;
    const su = (s.unit!=null) ? s.unit : meta.unit;
    rows+=`<div><i class="dot" style="background:${s.color};display:inline-block;margin-right:6px"></i>${s.label} : <b>${fmtWith(p.v,sd)}</b> ${su.split(" ")[0]}</div>`;
  }
...
function renderRecruiters(){
  ...
  const body = fr ? `
      ...
      <div class="m-notes">
        ...
        <ul>
          <li>Marwane Benaddi, étudiant à l'EDHEC Business School (BBA3), pendant un stage de finance de marché sur le desk Rates de BMCE Capital Markets.</li>
      ...
      <div class="m-notes">
        ...
        <ul>
          <li>Marwane Benaddi, EDHEC Business School student (BBA3), during a financial markets internship on the Rates desk at BMCE Capital Markets.</li>
